...
To illustrate higher-frequency estimates, we are going to download intraday data from Alpha Vantage. You must register with Alpha Vantage in order to download their data, but the one-time registration is fast and free. Register at https://www.alphavantage.co/ to receive your key. A sample key is provided below. You can set the API key globally as follows:

```{r}
setDefaults(getSymbols.av, api.key = "4FC5HHAXTSE5SUTL")
```

Download minute data for Microsoft in the last 15 days:
